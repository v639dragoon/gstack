...
for (const line of rawStat.split('\n')) {
  ...
  if (!m) continue;
  addFile(m[1] === '-' ? 0 : parseInt(m[1], 10), m[2] === '-' ? 0 : parseInt(m[2], 10), m[3], false);
}
...
const scope: Record<string, boolean | string | null> = {
  ...
  config: false, migrations: false, api: false, auth: false, error: null,
};
...
let policy: { present: boolean; path: string | null; sha256: string | null; version: number | null } = {
  present: false, path: null, sha256: null, version: null,
};
...
const loadBearingMatches = pol ? filePaths.filter((p) => matchAny(p, pol!.load_bearing_docs ?? [])) : [];
...
//   B  everything else
const trivialRe = /(\.md$|\.(css|scss|less|sass|pcss)$|(^|\/)(CHANGELOG|TODOS|VERSION|LICENSE)[^/]*$|\.(test|spec)\.[^/]+$|(^|\/)(tests?|__tests__|e2e|spec|design-reviews|eng-reviews)\/)/;
let tier: string | null = null;
let tierRule: string | null = null;
if (pol) {
  if (dSurfaceMatches.length) {
    tier = 'D'; tierRule = `d_surface:${dSurfaceMatches[0]}`;
  } else if (files.length === 0) {
    tier = 'C'; tierRule = 'empty-file-list (fail-upward)';
  } else if (scope.error) {
    tier = 'C'; tierRule = `scope-error:${scope.error} (fail-upward)`;
  } else if (filePaths.every((p) => trivialRe.test(p))) {
    if (loadBearingMatches.length) {
      tier = 'B'; tierRule = `load-bearing-doc:${loadBearingMatches[0]}`;
    } else {
      tier = 'A'; tierRule = 'all-files-trivial-shaped';
    }
  } else if (scope.backend || scope.migrations || scope.api) {
    tier = 'C';
    tierRule = `scope:${scope.migrations ? 'migrations' : scope.backend ? 'backend' : 'api'}`;
  } else {
    tier = 'B'; tierRule = 'routine-product-change';
  }
}

const docFpInput =
  filePaths.join('\n') + '\n--base--\n' + env.GDM_BASE + '\n--doc-impact--\n' +
  docImpactMatches.flatMap((r) => r.paths).sort().join('\n');
...
if (fs.existsSync(outPath)) {
  try {
    ...
    reused = existing.policy?.sha256 === policy.sha256; // policy changed → recompute
  } catch {
    reused = false;
  }
}
if (!reused) fs.writeFileSync(outPath, JSON.stringify(manifest, null, 2) + '\n');

// Opportunistic cleanup: dirty-tree churn mints a file per content state;
// keep the 20 newest, they are all regenerable.
const entries = fs.readdirSync(env.GDM_MANIFEST_DIR!)
