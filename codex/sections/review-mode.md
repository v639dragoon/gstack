...
```bash
~/.claude/skills/gstack/bin/gstack-review-log '{"skill":"codex-review","timestamp":"TIMESTAMP","status":"STATUS","gate":"GATE","findings":N,"findings_fixed":N,"effort":"EFFORT","effort_source":"EFFORT_SOURCE","commit":"'"$(git rev-parse --short HEAD)"'","completed":COMPLETED,"converged":CONVERGED}' --finish CODEX_REVIEW_START
```
...
EFFORT (the model_reasoning_effort actually used), EFFORT_SOURCE ("default", or
"user-override" when `--xhigh` was applied — then ALSO add
`"effort_reason":"<the user's stated reason, or 'user --xhigh flag, no reason
given'>"` per the recording rule in the effort-override section above).
