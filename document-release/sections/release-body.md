...
eval "$("$GSTACK_BIN/gstack-codex-model" resolve --voice 'doc-release' --effort medium)" || exit 1
_OUTSIDE_T0=$(date +%s)
...
_row() { "$GSTACK_BIN/gstack-voice-row" 'document-release' 'doc-release' "$1" "$CODEX_MODEL" "$CODEX_MODEL_SOURCE" "$CODEX_EFFORT" "$_OUTSIDE_T0"; }
