...
- Dual voices: always run BOTH Claude subagent AND Codex if available (P6).
  ...
  **Codex DX voice** (via Bash):
  ...
  Be adversarial. Think like a developer who is evaluating this against 3 competitors." -C "$_REPO_ROOT" -s read-only -c 'model_reasoning_effort="medium"' -c 'web_search="cached"' < /dev/null
  _CODEX_EXIT=$?
