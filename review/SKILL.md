...
**Test stub handling:** Only a BLOCKING finding with a `test_stub` includes it
in the ASK item. Advisory test stubs remain unfixed with their finding. Show
the proposed test file path and code. If approved, write the fix + test file. Derive the test file path from
...
1. **VALID & ACTIONABLE comments:** Include them in the same classification.
Only BLOCKING comments enter the ASK/fix flow (A: Fix it now, B: Acknowledge,
C: False positive); advisory comments remain under `## Advisories (not fixed)`.
If the user chooses A for a blocker, reply using the **Fix reply template**.
If the user chooses C, use the **False Positive reply template** and save it
to both per-project and global greptile-history.
...
- `quality_score` = the PR Quality Score computed in Step 4.6.
