...
${ctx.paths.binDir}/gstack-review-budget resume "$RUN_ID"
...
first the \`codex-structured\` slot, if planned, as the Step ${ship ? '11' : '5.7'} block run in a
...
(\`${ctx.paths.binDir}/gstack-wtree\` still prints \`MANIFEST_WTREE\`); if it
...
\`\`\`bash
${ctx.paths.binDir}/gstack-review-budget complete "$RUN_ID" --cycle <n>${ship ? ' --require-audits' : ''}
\`\`\`
${ship ? `
